...
def _seed_users(conn):
    """Inserta usuarios mínimos para las pruebas."""
    conn.execute(
        "INSERT OR IGNORE INTO users (id,username,email,password_hash,is_active) "
        "VALUES (1,'piero','piero@test.com','hash_piero',1)"
    )
    conn.execute(
        "INSERT OR IGNORE INTO users (id,username,email,password_hash,is_active) "
        "VALUES (2,'ana','ana@test.com','hash_ana',1)"
    )
    conn.execute(
        "INSERT OR IGNORE INTO users (id,username,email,password_hash,is_active) "
        "VALUES (3,'jose','jose@test.com','hash_jose',1)"
    )
    conn.commit()
